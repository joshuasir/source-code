...
void PromptUser(){
	...
	while(!feof(fp)){
		fscanf(fp,"%[^#]#%[^#]#%[^#]#%[^#]#%[^\n]\n",user[dex].email,user[dex].password,user[dex].name,user[dex].gender,user[dex].comment);
		dex++;
	}
	CUser=dex;
	fclose(fp);
...
void getFriends(int mind){
	...
	str[len]='\0';
		...
		while(!feof(fp)){
		fscanf(fp,"%[^#]#%[^#]#%[^\n]\n",user[mind].friends[dex].email,user[mind].friends[dex].name,user[mind].friends[dex].gender);
		dex++;
		}
		user[mind].end=dex;
		fclose(fp);
...
int main(){
	...
	switch(choose){
		...
		for(;;){
		system("cls");
		printf("Welcome %s\nStatus :\n%s\n",user[cookie].name,user[cookie].comment);
